...
def render_feature2(roboflow_detector):
    """Render Feature 2 - Enhanced Roboflow Real-time Detection"""
    st.markdown("""
    <div style="background: linear-gradient(135deg, #ff6b6b 0%, #ee5a52 100%); color: white; text-align: center; padding: 4rem 2rem; border-radius: 25px; margin-bottom: 2rem;">
        <h1>Feature 2: Enhanced Live Detection</h1>
        <p style="font-size: 1.4rem; margin: 0;">Professional Roboflow Pipeline with Integrated Freshness Analysis</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col3:
        pipeline_status = detector.data_store.detection_pipeline_status
        status_text = "Running" if pipeline_status['is_running'] else "Stopped"
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{status_text}</div>
            <div class="metric-label">Live Detection</div>
        </div>
        """, unsafe_allow_html=True)
    
    with col4:
        total_live = detector.data_store.system_stats['total_live_detections']
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{total_live}</div>
            <div class="metric-label">Live Detections</div>
        </div>
        """, unsafe_allow_html=True)
    
    # Pipeline status information
    if pipeline_status['current_model']:
        st.markdown(f"""
        <div class="pipeline-status">
            <h3>Active Model: {pipeline_status['current_model']}</h3>
            <p>Last Detection: {pipeline_status['last_detection_time'].strftime('%H:%M:%S') if pipeline_status['last_detection_time'] else 'None'}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if INFERENCE_AVAILABLE:
        st.markdown("""
        <div class="feature-card">
            <h3>Enhanced Real-time Detection System</h3>
            <p>This enhanced pipeline integrates the advanced detection capabilities from check.py with:</p>
            <ul>
                <li><strong>Multiple Model Fallback:</strong> Automatically tries different Roboflow models for optimal performance</li>
                <li><strong>Enhanced Visualization:</strong> Color-coded confidence levels with detailed information overlays</li>
                <li><strong>Integrated Freshness Analysis:</strong> Real-time freshness assessment for each detected part</li>
                <li><strong>Performance Monitoring:</strong> FPS tracking and detection statistics</li>
                <li><strong>Data Integration:</strong> All detections are logged to the analytics system</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="live-detection-controls">
            <h3>Live Detection Controls</h3>
            <p>Enhanced pipeline with integrated freshness analysis and performance monitoring</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            if st.button("Start Enhanced Detection", type="primary", key="start_enhanced"):
                if camera_ok:
                    st.info("Starting enhanced camera detection...")
                    st.info("A new window will open with live detection. Features include:")
                    st.info("- Color-coded confidence levels")
                    st.info("- Real-time freshness analysis")
                    st.info("- Performance metrics display")
                    st.info("- Enhanced part name overlays")
                    st.info("Press 'q' in the detection window to stop.")
                    ...
                    try:
                        ...
                        st.success("Enhanced detection started! Data is being logged to analytics.")
                    except Exception as e:
                        st.error(f"Failed to start enhanced detection: {str(e)}")
                else:
                    st.error(f"Camera not ready: {camera_msg}")
        
        with col2:
            if st.button("Stop Detection", key="stop_enhanced"):
        ...
        with col3:
            if st.button("Test Enhanced System", key="test_enhanced"):
                with st.spinner("Testing enhanced system..."):
                    # Test camera
                    camera_ok, camera_msg = detector.test_camera()
                    if camera_ok:
                        st.success(f"Camera test passed: {camera_msg}")
                    else:
                        st.error(f"Camera test failed: {camera_msg}")
        
        # Detection statistics
        if detector.data_store.real_time_detections:
            st.markdown("### Enhanced Detection Statistics")
            ...
            with col1:
                ...
                st.markdown(f"- Frame Rate: {stats['fps']:.1f} FPS")
                st.markdown(f"- Frames Processed: {stats['frame_count']}")
            ...
            with col3:
                ...
                for category, count in stats['freshness_distribution'].items():
                    if count > 0:
                        st.markdown(f"- {category}: {count}")
        
        # Enhanced detection info
        st.markdown("### Detection System Information")
        
        with st.expander("Enhanced Detection Features"):
            st.markdown("""
            **Color-Coded Confidence Levels:**
            - Green boxes: Very high confidence (≥80%)
            - Yellow boxes: High confidence (60-79%)
            - Orange boxes: Medium confidence (40-59%)
            - Red boxes: Low confidence (<40%)
            
            **Enhanced Overlays:**
            - Part names appear directly on bounding boxes
            - Confidence percentages shown next to detections
            - Freshness indicators integrated ([FRESH], [CAUTION], [SPOILED])
            - Detection summary panel with live statistics
            
            **Performance Features:**
            - Real-time FPS monitoring
            - Frame count tracking
            - Detection statistics overlay
            - Enhanced error handling and model fallback
            
            **Data Integration:**
            - All detections logged to analytics system
            - Freshness analysis for each detected part
            - Performance metrics tracked
            - Real-time data sharing across features
            """)
    
    else:
        st.markdown("""
        <div class="feature-card">
            <h3>Setup Required for Enhanced Detection</h3>
            <p><strong>Install inference package:</strong></p>
            <code>pip install inference</code>
            <p>This will enable the enhanced real-time detection pipeline with all advanced features.</p>
        </div>
        """, unsafe_allow_html=True)

def render_feature3(data_store):
    """Render Feature 3 - Enhanced Integrated Analytics Dashboard"""
    st.markdown("""
    <div style="background: linear-gradient(135deg, #4ecdc4 0%, #44a08d 100%); color: white; text-align: center; padding: 4rem 2rem; border-radius: 25px; margin-bottom: 2rem;">
        <h1>Feature 3: Enhanced Analytics Dashboard</h1>
        <p style="font-size: 1.4rem; margin: 0;">Real-time Analytics from All System Features with Live Detection Data</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    pipeline_status = analytics_data['pipeline_status']
    
    if st.button("Refresh Enhanced Dashboard", type="primary"):
        st.rerun()
    ...
    with col3:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{stats['total_live_detections']}</div>
            <div class="metric-label">Live Detections</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col5:
        pipeline_status_text = "ACTIVE" if stats['live_detection_active'] else "INACTIVE"
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{pipeline_status_text}</div>
            <div class="metric-label">Live Pipeline</div>
        </div>
        """, unsafe_allow_html=True)
    
    # Enhanced recent activity
    st.markdown("### Enhanced Activity Monitor (Last Hour)")
    ...
    with col3:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{recent['live_detections_last_hour']}</div>
            <div class="metric-label">Live Detections</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # Enhanced analytics charts with live detection data
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("#### Part Detection Distribution")
        if analytics_data['part_detections'] or analytics_data['real_time_detections']:
            part_counts = {}
            
            # Combine file upload detections
            for detection in analytics_data['part_detections']:
                part = detection['detected_part']
                part_counts[part] = part_counts.get(part, 0) + 1
            
            # Add live detection data
            for detection in analytics_data['real_time_detections']:
                class_name = detection.get('class', 'Unknown')
                part_counts[class_name] = part_counts.get(class_name, 0) + 1
            
            if part_counts:
                fig = px.bar(
                    x=list(part_counts.keys()),
                    y=list(part_counts.values()),
                    title="All Detected Chicken Parts",
                    labels={'x': 'Part Type', 'y': 'Count'},
                    color=list(part_counts.values()),
            ...
            st.info("Use Features 1 or 2 to generate part detection data.")
    
    with col2:
        st.markdown("#### Combined Freshness Analysis")
        if analytics_data['freshness_analyses'] or analytics_data['real_time_detections']:
            ...
            # Add live detection freshness data
            for detection in analytics_data['real_time_detections']:
                if detection.get('freshness'):
            ...
            if sum(freshness_counts.values()) > 0:
                fig = px.pie(
                    ...
                    title="Combined Freshness Distribution",
                    color_discrete_map={
                        'Fresh': '#00b894',
                        'Questionable': '#fdcb6e',
                        'Spoiled': '#e17055'
            ...
            st.info("Use Features 2 or 4 to generate freshness analysis data.")
    
    # Live detection timeline
    if analytics_data['real_time_detections']:
        st.markdown("#### Live Detection Timeline")
        
        # Prepare enhanced timeline data
        timeline_data = []
        for detection in analytics_data['real_time_detections'][-30:]:  # Last 30 detections
            freshness_category = "Unknown"
            if detection.get('freshness'):
                freshness_category = detection['freshness']['freshness_category']
        ...
        if timeline_data:
            ...
            # Enhanced scatter plot with freshness color coding
            fig = px.scatter(
                df,
                x='timestamp',
                y='confidence',
                color='freshness',
                size='confidence',
                hover_data=['type', 'confidence', 'freshness'],
                title="Live Detection Timeline with Freshness Analysis",
                color_discrete_map={
                    'Fresh': '#00b894',
                    'Questionable': '#fdcb6e',
                    'Spoiled': '#e17055',
    ...
    st.markdown("### Enhanced System Performance Summary")
    ...
    performance_data = {
        ...
        'Feature 2 (Enhanced Live Detection)': {
            'status': 'Active' if pipeline_status['is_running'] else 'Waiting for activation',
            'data_points': len(analytics_data['real_time_detections']),
            'last_activity': pipeline_status['last_detection_time'].strftime('%Y-%m-%d %H:%M:%S') if pipeline_status['last_detection_time'] else 'None',
            'current_model': pipeline_status['current_model'] or 'None'
        },
    ...
    for feature, data in performance_data.items():
        with st.expander(f"{feature} - Status: {data['status']}"):
            ...
            if 'current_model' in data:
                st.markdown(f"**Current Model:** {data['current_model']}")
    
    # Enhanced data export option
    st.markdown("### Enhanced Data Export")
    if st.button("Export Complete Analytics Data"):
        export_data = {
        ...
        st.json(export_data)
        st.success("Complete analytics data exported successfully!")
...
def main():
    """Enhanced main application with integrated features"""
    st.set_page_config(
        page_title="AUTOPACK AI - Enhanced Integrated System",
        page_icon="🚀",
        layout="wide"
    )
    ...
    <div class="enhanced-header">
        <h1 class="enhanced-title">🚀 AUTOPACK AI - ENHANCED INTEGRATED SYSTEM</h1>
        <p class="enhanced-subtitle">Professional Detection Pipeline with Advanced Analytics Integration</p>
        <p>Feature 1: Part Detection | Feature 2: Enhanced Live Detection | Feature 3: Real-time Analytics | Feature 4: Freshness Analysis</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if 'enhanced_roboflow_detector' not in st.session_state:
        st.session_state.enhanced_roboflow_detector = EnhancedRoboflowDetector(st.session_state.data_store)
    ...
    pipeline_status = st.session_state.data_store.detection_pipeline_status
    
    # Pipeline status indicator
    pipeline_status_text = "LIVE DETECTION ACTIVE" if pipeline_status['is_running'] else "LIVE DETECTION READY"
    pipeline_color = "#00b894" if pipeline_status['is_running'] else "#667eea"
    
    st.markdown(f"""
    <div class="feature-card" style="text-align: center;">
        <h3>Enhanced Integrated System Status</h3>
        <div style="margin: 2rem 0;">
            <span class="status-excellent">Parts Detected: {data_stats['total_parts_detected']}</span>
            <span class="status-excellent">Live Detections: {data_stats['total_live_detections']}</span>
        ...
        <div style="background: {pipeline_color}; color: white; padding: 1rem 2rem; border-radius: 15px; margin: 1rem 0;">
            <h4>{pipeline_status_text}</h4>
            {f"<p>Model: {pipeline_status['current_model']}</p>" if pipeline_status['current_model'] else "<p>Multiple models available</p>"}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if current_feature == 'Feature 1':
        ...
        render_feature2(st.session_state.enhanced_roboflow_detector)
    ...
    <div style="text-align: center; padding: 3rem 0; color: #666;">
        <h2 style="color: #667eea;">AUTOPACK AI - Enhanced Professional System</h2>
        <p style="font-size: 1.2rem;">Feature 1: File-based Part Detection | Feature 2: Enhanced Live Detection Pipeline | Feature 3: Real-time Analytics Dashboard | Feature 4: Advanced Freshness Analysis</p>
        <p>Enhanced integration with live detection capabilities, real-time freshness analysis, and comprehensive data logging</p>
        <div style="margin: 2rem 0; padding: 1rem; background: #f8f9fa; border-radius: 10px;">
            <strong>Enhanced Features from check.py Integration:</strong><br>
            • Multiple model fallback system • Enhanced visualization overlays • Real-time freshness analysis<br>
            • Performance monitoring • Color-coded confidence levels • Comprehensive data logging
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if __name__ == "__main__":
    try:
        ...
        st.markdown("""
        <div style="text-align: center; padding: 2rem; background: #fee; border-radius: 10px; margin: 2rem 0;">
            <h3>System Recovery</h3>
            <p>If you encounter issues:</p>
            <p>1. Check camera permissions and connections</p>
            <p>2. Ensure required packages are installed: pip install inference opencv-python streamlit</p>
            <p>3. Verify internet connection for Roboflow API access</p>
            <p>4. Refresh the page to reset the system</p>
        </div>
        """, unsafe_allow_html=True)
                        st.markdown
...
- Feature 2: Enhanced Roboflow real-time detection pipeline (integrated from check.py)
...
def create_navigation():
    """Create enhanced feature navigation"""
    st.markdown("""
    <div class="feature-card">
        <h2 style="text-align: center; color: #667eea;">AUTOPACK AI Enhanced Features</h2>
        <p style="text-align: center; color: #7f8c8d; font-size: 1.2rem;">Integrated Detection & Analysis System with Live Processing</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    features = {
        'Feature 1': {'title': 'Part Detection', 'desc': 'Upload & Analyze'},
        'Feature 2': {'title': 'Live Detection', 'desc': 'Enhanced Pipeline'},
        'Feature 3': {'title': 'Analytics Hub', 'desc': 'Real-time Dashboard'},
    ...
    return current
        self.detection_pipeline_status = {
            'is_running': False,
            'current_model': None,
            'fps': 0,
    ...
    def add_realtime_detection(self, detection_data):
        ...
        self.system_stats['total_live_detections'] += 1
        self.detection_pipeline_status['last_detection_time'] = datetime.now()
    ...
    def update_pipeline_status(self, is_running, model_id=None):
        """Update live detection pipeline status"""
        self.detection_pipeline_status['is_running'] = is_running
        self.system_stats['live_detection_active'] = is_running
        if model_id:
            self.detection_pipeline_status['current_model'] = model_id
    ...
    def _update_average_confidence(self):
        ...
        for rt_detection in self.real_time_detections:
            if 'confidence' in rt_detection:
                all_confidences.append(rt_detection['confidence'])
    ...
    def get_analytics_data(self):
        ...
        return {
            ...
            'pipeline_status': self.detection_pipeline_status
        }
    ...
    def _get_recent_activity(self):
        ...
        recent_live = len([d for d in self.real_time_detections if d.get('timestamp', datetime.min) > recent_time])
        
        return {
            'parts_last_hour': recent_parts,
            'freshness_checks_last_hour': recent_freshness,
            'live_detections_last_hour': recent_live,
            'total_activity': recent_parts + recent_freshness + recent_live
        }
...
class EnhancedRoboflowDetector:
    """Enhanced Roboflow detection system integrated from check.py"""
    ...
    def __init__(self, data_store):
        ...
        # Enhanced model configurations from check.py
        self.model_configs = [
            {"model_id": "hendrickworkspace/custom-workflow", "confidence": 0.25},
            {"model_id": "hendrickworkspace/chicken-mx39r-oznps-instant-1", "confidence": 0.25},
            {"model_id": "chicken-mx39r-oznps/4", "confidence": 0.25},
            {"model_id": "chicken-mx39r-oznps/3", "confidence": 0.25},
            {"model_id": "chicken-mx39r-oznps/2", "confidence": 0.25},
            {"model_id": "chicken-mx39r-oznps/1", "confidence": 0.25},
        ]
    ...
    def test_camera(self):
        """Enhanced camera testing from check.py"""
        try:
            ...
            print(f"✅ Camera working! Resolution: {resolution}")
            cap.release()
    ...
    def custom_render_with_confidence(self, predictions, video_frame):
        """Enhanced rendering function from check.py with confidence levels and freshness"""
        # Get the frame
        frame = video_frame.image.copy() if hasattr(video_frame, 'image') else video_frame.copy()
        
        # Add timestamp
        cv2.putText(frame, f"Live Detection: {time.strftime('%H:%M:%S')}", (10, 30), 
        ...
        try:
            ...
            if pred_list and len(pred_list) > 0:
                print(f"🔍 Found {len(pred_list)} chicken part predictions")
                ...
                for i, prediction in enumerate(pred_list):
                    try:
                        ...
                        if hasattr(prediction, 'x'):
                            ...
                            class_name = getattr(prediction, 'class_name', getattr(prediction, 'class', 'Chicken Part'))
                            ...
                            height = int(prediction['height'])
                            confidence = prediction['confidence']
                            class_name = prediction.get('class_name', prediction.get('class', 'Chicken Part'))
                        ...
                        if 'Chicken' not in display_name:
                            display_name = f"Chicken {display_name}"
                        ...
                        # Draw chicken part name ON the bounding box
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        font_scale = 0.8
                        thickness = 2
                        
                        # Part name label
                        label_text = display_name + freshness_indicator
                        (text_w, text_h), _ = cv2.getTextSize(label_text, font, font_scale, thickness)
                        label_bg_y = y1 - 10 if y1 > text_h + 20 else y2 + 10
                        
                        # Draw part name background
                        cv2.rectangle(frame, (x1, label_bg_y - text_h - 10), 
                                     (x1 + text_w + 10, label_bg_y + 5), color, -1)
                        
                        # Draw part name text
                        cv2.putText(frame, label_text, (x1 + 5, label_bg_y - 5), 
                                   font, font_scale, (255, 255, 255), thickness)
                        
                        # Draw confidence NEXT TO the bounding box
                        confidence_text = f"{confidence:.1%}"
                        (conf_w, conf_h), _ = cv2.getTextSize(confidence_text, font, 0.7, 2)
                        
                        # Position confidence to the right of the box
                        conf_x = x2 + 10
                        conf_y = y1 + (y2 - y1) // 2 + conf_h // 2
                        
                        # If confidence goes off screen, put it on the left
                        if conf_x + conf_w > frame_width - 10:
                            conf_x = x1 - conf_w - 10
                            if conf_x < 0:
                                conf_x = x1 + 10  # Put inside box if nowhere else fits
                        
                        # Draw confidence background
                        cv2.rectangle(frame, (conf_x - 5, conf_y - conf_h - 5), 
                                     (conf_x + conf_w + 5, conf_y + 5), color, -1)
                        
                        # Draw confidence text
                        cv2.putText(frame, confidence_text, (conf_x, conf_y), 
                                   font, 0.7, (255, 255, 255), 2)
                        
                        # Store detected part info
                        ...
                        print(f"✅ {display_name}: {confidence:.1%} ({conf_level}){freshness_indicator}")
                ...
                print("No chicken parts detected in this frame")
        
        except Exception as e:
            print(f"❌ Error processing predictions: {e}")
        
        # Add detection summary panel
        if detected_parts:
            panel_y = frame.shape[0] - min(160, 40 + len(detected_parts) * 30)
            panel_height = min(150, 30 + len(detected_parts) * 30)
            
            # Draw summary panel background
            cv2.rectangle(frame, (10, panel_y), (500, frame.shape[0] - 10), (0, 0, 0), -1)
            cv2.rectangle(frame, (10, panel_y), (500, frame.shape[0] - 10), (255, 255, 255), 2)
            
            # Panel title
            cv2.putText(frame, f"🔍 DETECTED PARTS ({detection_count})", (20, panel_y + 25), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            
            # List detected parts with freshness
            for i, part in enumerate(detected_parts[:5]):  # Show max 5 parts
                freshness_text = ""
                if part['freshness']:
                    freshness_text = f" [{part['freshness']['freshness_category']}]"
                
                part_text = f"• {part['name']}: {part['confidence']:.1%}{freshness_text}"
                cv2.putText(frame, part_text, (25, panel_y + 50 + i * 22), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.45, part['color'], 1)
        ...
        # Add detection count and FPS to top
        cv2.putText(frame, f"Parts: {detection_count} | FPS: {fps:.1f}", (10, 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0) if detection_count > 0 else (0, 0, 255), 2)
        
        # Add system info
        cv2.putText(frame, "AUTOPACK AI - Enhanced Detection System", (10, frame.shape[0] - 50), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        ...
        # Display the frame
        cv2.imshow("🔍 AUTOPACK AI - Live Chicken Parts Detection with Freshness", frame)
    ...
    def start_detection_pipeline(self):
        """Enhanced detection pipeline startup from check.py"""
        if not INFERENCE_AVAILABLE:
            return False, "Inference package not available. Install with: pip install inference"
        
        print("🔥" + "="*50 + "🔥")
        print("   AUTOPACK AI - LIVE DETECTION SYSTEM")
        print("🔥" + "="*50 + "🔥")
        
        # Step 1: Test camera
        camera_ok, camera_msg = self.test_camera()
        if not camera_ok:
            print("Camera test failed! Please check your webcam.")
            return False, f"Camera error: {camera_msg}"
        
        # Step 2: Try different model versions
        pipeline = None
        
        for config in self.model_configs:
            try:
                print(f"\n🔄 Trying model: {config['model_id']}")
                ...
                pipeline = InferencePipeline.init(
                    ...
                    iou_threshold=0.5,
                    max_fps=30,  # Limit FPS for better performance
                )
                
                print(f"✅ Successfully initialized with {config['model_id']}")
                self.pipeline = pipeline
                self.data_store.update_pipeline_status(True, config['model_id'])
                break
                
            except Exception as e:
                print(f"❌ Failed with {config['model_id']}: {e}")
                continue
        ...
        if pipeline is None:
            ...
            print("2. Verify Roboflow API key")
            print("3. Check if model exists in workspace")
            print("4. Try running: pip install --upgrade inference")
            self.data_store.update_pipeline_status(False)
            return False, "All model configurations failed"
        
        print("\n🚀 STARTING LIVE DETECTION...")
        print("📋 DETECTION INFO:")
        print("   • Green boxes: Very high confidence (≥80%)")
        print("   • Yellow boxes: High confidence (60-79%)")
        print("   • Orange boxes: Medium confidence (40-59%)")
        print("   • Red boxes: Low confidence (<40%)")
        print("   • Part names appear ON the bounding boxes")
        print("   • Confidence percentages appear NEXT TO boxes")
        print("   • Freshness analysis integrated for each detection")
        print("\n💡 TIPS:")
        print("   • Hold chicken parts clearly in camera view")
        ...
        try:
            ...
            return True, "Detection interrupted by user"
        except Exception as e:
            print(f"\n❌ Pipeline error: {e}")
